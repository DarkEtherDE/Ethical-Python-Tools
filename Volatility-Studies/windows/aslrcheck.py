...
def check_aslr(pe):#Determine if the file was compiled in Dynamic or not, as well as if it 
    ...
    return aslr
class AslrCheck(interfaces.plugin.PluginInterface):#creation of inheritable class
    @classmethod
    def get_requirements(cls):#Requirements (translation layer for memory, windows symbol table, plugin specification, and specify an optional specification for specific PIDs)
        return[
    ...
    def _generator(self, procs): #define generator (secondary function to run to speed up process)
        ...
        procnames = list()
        for proc in procs:
            procname = proc.ImageFileName.cst("string", max_length=proc.ImageFileName.vol.count, errors='replace')
